...
class StockController extends Controller
{
    public function index(Request $request)
    {
            ...
            if($port_array) {
                foreach($port_unique_name as $key=>$row){
                    ...
                    for ($i=0; $i <count($port_array) ; $i++) { 
                        ...
                        if(strtolower($new_obj_arr[0]) == $row){
        ...
        if(!is_null($request->from_year)){
            $vehicle_data = $vehicle_data->whereRaw('CONVERT(SUBSTR(`vehicle`.registration, 1, 4), SIGNED) >= ?', $request->from_year);
    ...
    public function select_port(Request $request){ 
        ...
        $port_list = [];
        if($port->port){
            $port_unique_name = array_values(array_unique(array_map('strtolower', json_decode($port->port, true))));
            $port_array = json_decode($port->port_array, true);
            if($port_array) {
                foreach($port_unique_name as $key=>$row){
                    $get_last_body = [];
                    for ($i= 0; $i < count($port_array); $i++) { 
